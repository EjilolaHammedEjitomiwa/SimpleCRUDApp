...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        main_searchEditText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if (s!!.isNotEmpty()) {
                    searchSubscribers().execute(s.toString())
                }else{
                    showAllSubscribers().execute()
                }
            }
    ...
    inner class searchSubscribers() : AsyncTask<String, Void, Void>() {
        override fun doInBackground(vararg p0: String): Void? {
            val searchText = "%"+p0[0]+"%"
            val searchedSubscribers = dao!!.searchSubscriber(searchText)
            allSubscriber.clear()
            if (searchedSubscribers.isNotEmpty()) {
                for (i in searchedSubscribers) {
                    allSubscriber.add(i)
                }
                allSubscriber.reverse()
                Log.i("MYTAG", allSubscriber.toString())
